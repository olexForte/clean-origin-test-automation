...
public class FileManager {
    ...
    static String TEST_AUTOMATION_RESOURCES = "src/test/automation/resources/";
    ...
    public static File getFileFromScriptsDir(String fileName) {
        ...
        for(File file: files){
            if (file.getName().matches(fileName + "\\.(txt|json|ct)"))
                return file;
        }
        return null;
    }
    ...
    /**
     * Simple comparison // TODO update
     * @param actualData
     * @param expectedData
     * @return
     * @throws IOException
     */
    public static boolean compareFiles(String actualData, String expectedData, int startRow) throws IOException {
        boolean result = true;
        String line1;
        String line2;
        String[] actualLines = actualData.split("\n");
        String[] expectedLines = expectedData.split("\n");
        for(int index = startRow; index < expectedLines.length; index++){
            line1 = actualLines[index];
            boolean wasFound = false;
            for(int i = startRow; i < actualLines.length ; i++) {
                line2 = expectedLines[index];
                if (line1.equals(line2)) {
                    wasFound = true;
                    break;
                }
