...
public class RandomDataGenerator {
    ...
    public static String getRandomField(String parameters, String parameterDelimiter) {
        ...
        String NUMBER_RANGE_LABEL = "min_max";
        ...
        if (parameters.toLowerCase().contains(EMAIL_LABEL))
            return getRandomStringByTemplate(DEFAULT_TEMPLATE) + "@test.com";

        // number range
        if (parameters.toLowerCase().contains(NUMBER_RANGE_LABEL)) {
            String[] parts = parameters.split(parameterDelimiter);
            return getRandomNumber(Integer.valueOf(parts[1]), Integer.valueOf(parts[2]));
        }
    ...
    /**
     * Randomize file using current session id
     * @param dataFile
     * @return
     */
    public static String randomizeFile(File dataFile) throws IOException {
        String content = FileManager.getFileContent(dataFile);

        String updatedFileContent = "";
        for(String line: content.split("\n")) {
            updatedFileContent = updatedFileContent + randomizeString(line) + "\n";
        }

        String newFilePath = FileManager.OUTPUT_DIR + File.separator + dataFile.getName();
        FileManager.createFile(newFilePath, updatedFileContent);
        return newFilePath;
    }

    /**
     * Randomize string using current session id
     * @param data string to randomize
     * @return
     */
    public static String randomizeString(String data) throws IOException {
        String SESSION_MARKER = "$SESSION";
        String TIMESTAMP_MARKER = "$TIMESTAMP";
        String updatedData = data.replace(SESSION_MARKER, SessionManager.getSessionID());
        updatedData = updatedData.replace(TIMESTAMP_MARKER, RandomDataGenerator.getCurDateTime());
        updatedData = randomizeDates(updatedData);
        return updatedData;
    }

    private static String randomizeDates(String updatedData) {
        //${DATE:YYYY+1}
        String result = updatedData;
        return result;
    }
}
